...
export function AnniversaryDateFields({
    ...
    const [dateType, setDateType] = useState<"solar" | "lunar">(defaultDateType);

    // 如果是农历，需要计算对应的公历日期用于显示
    // 这里我们用 solarDate 字段，农历模式下让用户选择公历日期后自动转换
    const initialSolarDate = defaultDateType === "solar" ? defaultSolarDate : "";
        ...
        <>
            ...
            <div className="sm:col-span-2">
                <label className="block text-xs font-medium text-secondary mb-1.5">
                    {dateType === "solar" ? "日期（公历）" : "日期（选择公历日期，自动转换农历）"}
                </label>
                <SmartDateInput
                    name="solarDate"
                    dateType={dateType}
                    ...
                    className="h-10 bg-base/50"
                />
                {dateType === "lunar" && defaultLunarMonth && defaultLunarDay && (
